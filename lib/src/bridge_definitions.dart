...
part 'bridge_definitions.freezed.dart';
        ...
        abstract class SimpleAudio {
            Future<Player> newStaticMethodPlayer({ required List<MediaControlAction> actions ,required String dbusName ,int? hwnd ,dynamic hint });
...
FlutterRustBridgeTaskConstMeta get kPlaybackStateStreamStaticMethodPlayerConstMeta;

Stream<ProgressState> progressStateStreamStaticMethodPlayer({ dynamic hint });

FlutterRustBridgeTaskConstMeta get kProgressStateStreamStaticMethodPlayerConstMeta;
...
Future<ProgressState> getProgressMethodPlayer({ required Player that ,dynamic hint });
...
Future<void> seekMethodPlayer({ required Player that ,required int seconds ,dynamic hint });

FlutterRustBridgeTaskConstMeta get kSeekMethodPlayerConstMeta;

Future<void> setMetadataMethodPlayer({ required Player that ,required Metadata metadata ,dynamic hint });
...
FlutterRustBridgeTaskConstMeta get kNormalizeVolumeMethodPlayerConstMeta;
    ...
    OpaqueTypeFinalizer get ControlsFinalizer;
        ...
        @sealed class Controls extends FrbOpaque {
...
@freezed
                sealed class Callback with _$Callback  {
                     const factory Callback.error(  Error field0,) = Callback_Error;
...
/// a network stream.
const factory Error.networkStream({ /// The error message.
  required String message , }) = Error_NetworkStream;
 /// An error occurred when decoding the file.
const factory Error.decode({ /// The error message.
  required String message , }) = Error_Decode;
 /// An error occurred when trying to open a file.
const factory Error.open({ /// The error message.
  required String message , }) = Error_Open;
 /// An error occurred when trying to preload a file.
const factory Error.preload({ /// The error message.
  required String message , }) = Error_Preload;
                }
...
final Uint8List? artBytes;
                    ...
                    const Metadata({this.title ,this.artist ,this.album ,this.artUri ,this.artBytes ,});
...
final Controls controls;
                    ...
                    const Player({required this.bridge,required this.controls ,});

                static Future<Player> newPlayer({ required SimpleAudio bridge,required List<MediaControlAction> actions ,required String dbusName ,int? hwnd ,dynamic hint })=>bridge.newStaticMethodPlayer(actions:actions,dbusName:dbusName,hwnd:hwnd,hint: hint);

/// Stops media controllers and decoder threads.
static Future<void> dispose({ required SimpleAudio bridge,dynamic hint })=>bridge.disposeStaticMethodPlayer(hint: hint);

static Stream<PlaybackState> playbackStateStream({ required SimpleAudio bridge,dynamic hint })=>bridge.playbackStateStreamStaticMethodPlayer(hint: hint);

static Stream<ProgressState> progressStateStream({ required SimpleAudio bridge,dynamic hint })=>bridge.progressStateStreamStaticMethodPlayer(hint: hint);

static Stream<Callback> callbackStream({ required SimpleAudio bridge,dynamic hint })=>bridge.callbackStreamStaticMethodPlayer(hint: hint);

 Future<bool> isPlaying({ dynamic hint })=>bridge.isPlayingMethodPlayer(that: this, );

/// Returns `true` if there is a file preloaded for playback.
 Future<bool> hasPreloaded({ dynamic hint })=>bridge.hasPreloadedMethodPlayer(that: this, );

 Future<ProgressState> getProgress({ dynamic hint })=>bridge.getProgressMethodPlayer(that: this, );

/// Opens a file or network resource for reading and playing.
 Future<void> open({ required String path ,required bool autoplay ,dynamic hint })=>bridge.openMethodPlayer(that: this, path:path,autoplay:autoplay,);
...
/// the time spent loading between tracks (especially important
/// for streaming network files).
 Future<void> preload({ required String path ,dynamic hint })=>bridge.preloadMethodPlayer(that: this, path:path,);

/// Plays the preloaded item from `preload`. The file starts playing automatically.
 Future<void> playPreload({ dynamic hint })=>bridge.playPreloadMethodPlayer(that: this, );

 Future<void> play({ dynamic hint })=>bridge.playMethodPlayer(that: this, );

 Future<void> pause({ dynamic hint })=>bridge.pauseMethodPlayer(that: this, );

 Future<void> stop({ dynamic hint })=>bridge.stopMethodPlayer(that: this, );

 Future<void> loopPlayback({ required bool shouldLoop ,dynamic hint })=>bridge.loopPlaybackMethodPlayer(that: this, shouldLoop:shouldLoop,);

 Future<void> setVolume({ required double volume ,dynamic hint })=>bridge.setVolumeMethodPlayer(that: this, volume:volume,);

 Future<void> seek({ required int seconds ,dynamic hint })=>bridge.seekMethodPlayer(that: this, seconds:seconds,);

 Future<void> setMetadata({ required Metadata metadata ,dynamic hint })=>bridge.setMetadataMethodPlayer(that: this, metadata:metadata,);

 Future<void> normalizeVolume({ required bool shouldNormalize ,dynamic hint })=>bridge.normalizeVolumeMethodPlayer(that: this, shouldNormalize:shouldNormalize,);
...
final int duration;
                    ...
                    const ProgressState({required this.position ,required this.duration ,});
