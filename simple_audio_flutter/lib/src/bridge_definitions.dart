...
FlutterRustBridgeTaskConstMeta get kErrorStreamStaticMethodPlayerConstMeta;
...
@sealed class Player extends FrbOpaque {
...
/// a network stream.
const factory Error.networkStream(  String field0,) = Error_NetworkStream;
 /// An error occurred when decoding the file.
const factory Error.decode(  String field0,) = Error_Decode;
 /// An error occurred when trying to open a file.
const factory Error.open(  String field0,) = Error_Open;
 /// An error occurred when trying to preload a file.
const factory Error.preload(  String field0,) = Error_Preload;
                }
...
const factory PlaybackState.started(  Duration field0,) = PlaybackState_Started;
...
final Player internal;
                    ...
                    const Player({required this.bridge,required this.internal ,});

                static Future<Player> newPlayer({ required SimpleAudio bridge,dynamic hint })=>bridge.newStaticMethodPlayer(hint: hint);
...
static Stream<Error> errorStream({ required SimpleAudio bridge,dynamic hint })=>bridge.errorStreamStaticMethodPlayer(hint: hint);

 Future<PlaybackState> playbackState({ dynamic hint })=>bridge.playbackStateMethodPlayer(that: this, );

 Future<ProgressState> progress({ dynamic hint })=>bridge.progressMethodPlayer(that: this, );

/// Returns `true` if there is a file preloaded for playback.
 Future<bool> isPreloaded({ dynamic hint })=>bridge.isPreloadedMethodPlayer(that: this, );

 Future<bool> isLooping({ dynamic hint })=>bridge.isLoopingMethodPlayer(that: this, );

 Future<bool> isNormalizing({ dynamic hint })=>bridge.isNormalizingMethodPlayer(that: this, );

 Future<double> volume({ dynamic hint })=>bridge.volumeMethodPlayer(that: this, );
 ...
 Future<void> clearPreload({ dynamic hint })=>bridge.clearPreloadMethodPlayer(that: this, );
 ...
 Future<void> seek({ required Duration position ,dynamic hint })=>bridge.seekMethodPlayer(that: this, position:position,);
